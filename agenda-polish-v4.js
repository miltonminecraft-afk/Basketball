...
function injectCss(){
 if($('agendaPolishV4Css'))return;
 const s=document.createElement('style');s.id='agendaPolishV4Css';s.textContent=`
...
async function tagDays(root,map){
 ...
 for(const day of root.querySelectorAll('.day')){let date=day.dataset.agendaDate||'';if(!date){const mc=day.querySelector('[data-match-id]');if(mc)date=d(map.get(String(mc.dataset.matchId))?.date);if(!date){const tc=day.querySelector('[data-task-id]'),row=tc?taskMap.get(String(tc.dataset.taskId)):null;if(row)date=d(row.date)}}if(date)day.dataset.agendaDate=date}
 for(const card of root.querySelectorAll('article.event')){if(card.dataset.matchId)card.dataset.agendaTime=t(map.get(String(card.dataset.matchId))?.startTime);else if(card.dataset.taskId){const row=taskMap.get(String(card.dataset.taskId));card.dataset.agendaTime=t(row?.arrivalTime||row?.startTime)}}
}
...
async function render(force=false){
 ...
 try{
  ...
  const extraGuids=new Set([...c.memberTeams,...c.trainerTeams].map(x=>String(x.foy_team_guid||'')).filter(Boolean));if(sel?.teamGuid&&!allSelected)extraGuids.add(String(sel.teamGuid));
  await Promise.all([...extraGuids].map(teamMatches));let map=matchMap();await tagDays(root,map);root.querySelectorAll('.agenda-training,.agenda-extra-match').forEach(x=>x.remove());
  const existing=new Set([...root.querySelectorAll('[data-match-id]')].map(x=>String(x.dataset.matchId))),personalGuids=new Set([...c.memberTeams,...c.trainerTeams].map(x=>String(x.foy_team_guid||'')).filter(Boolean));
  ...
  const selectedTeam=c.allTeams.find(x=>String(x.foy_team_guid)===String(sel?.teamGuid||'')),relevantIds=new Set([...c.memberTeams,...c.trainerTeams].map(x=>x.id));if(selectedTeam)relevantIds.add(selectedTeam.id);
  const visibleSlots=c.slots.filter(slot=>allSelected||slot.team_id==null||!slot.teams||relevantIds.has(slot.team_id)),q=season(),today=new Date().toISOString().slice(0,10);
 ...
 }catch(e){console.warn('Agenda kon niet worden aangevuld',e)}finally{
  rendering=false;resumeObservers();
  document.dispatchEvent(new CustomEvent('basketball-agenda-polished'));
 }
}
function schedule(force=false,delay=220){clearTimeout(timer);timer=setTimeout(()=>render(force),delay)}
function installObservers(){agendaObserver=new MutationObserver(muts=>{if(rendering)return;if(muts.some(m=>m.addedNodes.length||m.removedNodes.length))schedule(false,180)});gamesObserver=new MutationObserver(muts=>{if(rendering)return;if(muts.some(m=>m.addedNodes.length||m.removedNodes.length))schedule(false,180)});resumeObservers()}
function init(){
