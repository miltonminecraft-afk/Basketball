...
let sb=null,timer=null,data=null,dataAt=0,member=null,memberAt=0,unread=new Set(),seen=new Set(),busy=false;
async function client(){if(sb)return sb;const mod=await import('https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm');sb=mod.createClient(U,K,{auth:{persistSession:true,detectSessionInUrl:true,autoRefreshToken:true}});sb.auth.onAuthStateChange(()=>{member=null;memberAt=0;unread.clear();seen.clear();schedule(true,80)});return sb}
...
async function seenEvents(force=false){seen=new Set();const m=await who(force);if(!m)return seen;const s=await client(),{data:rows,error}=await s.rpc('get_seen_task_changes');if(error)throw error;for(const row of rows||[])if(row.task_event_id)seen.add(String(row.task_event_id));return seen}
...
function css(){if($('taskChangeLabelsV4Css'))return;document.querySelector('#taskChangeLabelsV1Css')?.remove();document.querySelector('#taskChangeLabelsV2Css')?.remove();document.querySelector('#taskChangeLabelsV3Css')?.remove();const s=document.createElement('style');s.id='taskChangeLabelsV4Css';s.textContent=`.task-change-pill{display:inline-flex;align-items:center;margin-left:5px;padding:3px 6px;border-radius:999px;background:#fff0df;color:#9b4c00;font-size:8px;font-weight:900;line-height:1}`;document.head.appendChild(s)}
async function apply(force=false){if(busy)return;busy=true;try{css();const j=await tasks(force),selected=selectedPerson(),m=await who(force);if(selected==='__all__'){if(m)await seenEvents(force)}else await unreadEventsForSelected(force);const map=new Map((j?.tasks||[]).map(x=>[String(x.id),x]));for(const card of document.querySelectorAll('#agendaList article.event[data-task-id],#tasksList article.event[data-task-id]')){card.querySelectorAll('.task-change-pill,.task-change-note').forEach(x=>x.remove());const task=map.get(String(card.dataset.taskId));if(!task)continue;const eventId=String(task.taskEventId||''),changed=selected==='__all__'?!!task.changed&&(!m||!seen.has(eventId)):unread.has(eventId);if(!changed)continue;const badge=card.querySelector('.badge-task');if(badge){const pill=document.createElement('span');pill.className='task-change-pill';pill.textContent='Gewijzigd';badge.insertAdjacentElement('afterend',pill)}}}catch(e){console.warn('Taakwijzigingslabels',e)}finally{busy=false}}
function schedule(force=false,delay=80){clearTimeout(timer);timer=setTimeout(()=>apply(force),delay)}
function init(){css();const observer=new MutationObserver(()=>schedule(false,100));for(const id of ['agendaList','tasksList']){const el=$(id);if(el)observer.observe(el,{childList:true,subtree:true})}for(const ev of ['task-schedule-imported','staff-task-updated','basketball-task-change-read'])document.addEventListener(ev,()=>{data=null;dataAt=0;memberAt=0;unread.clear();seen.clear();schedule(true,40)});document.addEventListener('basketball-team-data-rendered',()=>schedule(false,80));$('personSelect')?.addEventListener('change',()=>schedule(true,60));schedule(true,700)}
