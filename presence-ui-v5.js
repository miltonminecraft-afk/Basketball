...
function isPlayedCard(card,m){
  if(m&&m.homeScore!==null&&m.homeScore!==undefined&&m.awayScore!==null&&m.awayScore!==undefined)return true;
  if(String(m?.status||'').toLowerCase()==='final')return true;
  return !!card?.querySelector('.badge-final,.score-line')||/\b(Uitslag|EINDSTAND)\b/i.test(card?.textContent||'');
}
function argonSideFromMatch(m){
  if(!m)return'';
  const home=norm(label(m,'home')),away=norm(label(m,'away'));
  if(home.startsWith('svargon'))return'home';
  if(away.startsWith('svargon'))return'away';
  return'';
}
function argonSideFromReport(j){
  const home=norm(j?.match?.home),away=norm(j?.match?.away);
  if(home.startsWith('svargon'))return'home';
  if(away.startsWith('svargon'))return'away';
  return'';
}
async function bondPlayedCount(id,side=''){
  const key=String(id||'');if(!/^\d+$/.test(key))return null;
  ...
  try{
    ...
    playedCountCache.set(key,value);
    return value;
  }catch(e){
...
function formatCardMeta(card,m,countText,mode){
  ...
  const address=addressText(m),href=mapsHref(m),signature=`${location}|${address}|${href}|${mode}|${countText||''}`;
...
async function refreshCountRules(){
  ...
  try{
    const resolved=await Promise.all(cards.map(async card=>{
      ...
      return{card,match,away:match?await isAwayMatch(match):false,played:isPlayedCard(card,match)};
    }));
    ...
    await Promise.all(resolved.map(async({card,match,away,played})=>{
      if(played){
        formatCardMeta(card,match,'','played');
        const side=argonSideFromMatch(match);
        const players=await bondPlayedCount(card.dataset.matchId,side);
        if(players!==null)formatCardMeta(card,match,`Spelers: ${players}`,'played');
        return;
      }
