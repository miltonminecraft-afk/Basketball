...
function loadLocalScript(src,globalName){
 ...
 return new Promise((resolve,reject)=>{
  ...
  s.onload=()=>window[globalName]?resolve(window[globalName]):reject(Error(`${globalName} kon niet worden gestart.`));
  s.onerror=()=>reject(Error('Importmodule kon niet worden geladen.'));
  document.head.appendChild(s);
 });
}
async function modules(){
 let P=parser();if(!P)P=await loadLocalScript('./task-import-parsers-v1.js?v=2','BasketballTaskImportParsers');
 let E=engine();if(!E)E=await loadLocalScript('./task-import-engine-v1.js?v=2','BasketballTaskImportEngine');
 return{E,P};
}
function render(){
...
async function handleFile(file){
 if(!file)return;
 const name=$('taskImportName'),box=$('taskImportPreview');
 if(name)name.textContent=file.name;
 if(box)box.innerHTML='<div class="club-empty">Bestand analyseren…</div>';
 toast(`${file.name} geselecteerd. Analyseren…`);
 try{
  const {E,P}=await modules();
  E.state.file=file;E.state.rows=[];E.state.resolved=[];E.state.missing=[];E.state.removed=[];
  E.state.rows=await P.parseFile(file);
  if(!E.state.rows.length)throw Error('Geen taakregels gevonden.');
  const liveBox=$('taskImportPreview');if(liveBox)liveBox.innerHTML=`<div class="club-empty">${E.state.rows.length} taakregels gevonden. Wedstrijden koppelen…</div>`;
  await E.loadData();
  E.recompute();render();
  toast(`${E.state.rows.length} taakregels gevonden.`);
 }catch(err){
  console.error(err);
  const E=engine();if(E){E.state.rows=[];E.state.resolved=[];E.state.missing=[];E.state.removed=[]}
  const liveBox=$('taskImportPreview');if(liveBox)liveBox.innerHTML=`<div class="error-box">${esc(err.message||'Bestand kon niet worden gelezen.')}</div>`;
  toast(err.message||'Bestand kon niet worden gelezen.');
 }
}
function css(){
 if($('taskImportV2Css'))return;
 const s=document.createElement('style');s.id='taskImportV2Css';s.textContent=`
...
function mount(){
 ...
 panel.innerHTML=`<h4>Takenschema importeren</h4><p>PDF of Excel. Eerst matchen op team + tegenstander + thuis/uit; datum, tijd en locatie zijn alleen hulpsignalen.</p><div class="task-import-picker"><button type="button" class="task-import-file-label" id="taskImportChoose">Bestand kiezen</button><input id="taskImportFile" type="file" aria-label="Takenschema kiezen" accept=".pdf,.xlsx,.xls,.xlsm,.csv,application/pdf,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"><span class="task-import-name" id="taskImportName">Nog geen bestand</span></div><div id="taskImportPreview"></div>`;
 if(head)head.insertAdjacentElement('afterend',panel);else root.prepend(panel);
 const E=engine();if(E?.state?.file){$('taskImportName').textContent=E.state.file.name;if(E.state.rows?.length)render()}
}
function init(){
 css();mount();document.addEventListener('admin-agenda-refreshed',mount);
 document.addEventListener('click',e=>{const b=e.target.closest?.('#taskImportChoose');if(!b)return;const input=$('taskImportFile');if(!input)return;input.value='';input.click()});
 document.addEventListener('change',e=>{const input=e.target;if(!(input instanceof HTMLInputElement)||input.id!=='taskImportFile')return;handleFile(input.files?.[0])});
}
