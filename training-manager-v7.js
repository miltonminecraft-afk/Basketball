...
function trainerSlots(memberId){if(trainerHasAllTeams(memberId))return slots.slice();const ids=trainerIds(memberId);return slots.filter(x=>ids.has(String(x.team_id)))}
function teamOptions(selected=''){return teams.filter(t=>norm(t.team_name)!=='alle teams').map(t=>`<option value="${esc(t.id)}" ${String(selected)===String(t.id)?'selected':''}>${esc(t.team_name)}</option>`).join('')}
function dayChecks(selected=[]){return Object.entries(DAYS).map(([n,d])=>`<label class="training-day"><input type="checkbox" name="trainingDay" value="${n}" ${selected.includes(Number(n))?'checked':''}>${d}</label>`).join('')}
