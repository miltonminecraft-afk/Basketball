...
let sb=null,ctxCache=null,ctxAt=0,presenceBusy=false,presenceTimer=0,countsBusy=false,countsTimer=0,headerBusy=false;
...
function label(m,side){
  return [m?.[`${side}TeamSponsorClubName`]||m?.[`${side}Organisation`]?.name,m?.[`${side}TeamName`]||m?.[`${side}Team`]?.name].filter(Boolean).join(' ').trim()||'Onbekend team';
}
function guid(m,side){return String(m?.[`${side}TeamGuid`]||m?.[`${side}Team`]?.guid||'')}
function shortDate(v){
  try{return new Intl.DateTimeFormat('nl-NL',{weekday:'short',day:'2-digit',month:'short',year:'numeric'}).format(new Date(`${d(v)}T12:00:00`))}
  catch{return d(v)}
}
function toast(msg){
...
async function client(){
  ...
  sb.auth.onAuthStateChange(()=>{ctxCache=null;ctxAt=0;matchCache.clear();schedulePresence(true);scheduleCounts()});
  return sb;
}
async function context(force=false){
  ...
  ctxCache={member,playerTeams,trainerTeams,trainerAll,allTeams};
  ctxAt=Date.now();
  return ctxCache;
}
async function teamMatches(teamGuid){
  if(matchCache.has(teamGuid))return matchCache.get(teamGuid);
  ...
  while(skip<total){
    ...
    all.push(...rows);total=Number(j?.totalCount)||rows.length;skip+=rows.length;
    if(!rows.length||rows.length<100)break;
  }
  matchCache.set(teamGuid,all);
  return all;
}
function isAwayForTeam(m,team){
  const tg=String(team.foy_team_guid||'');
  if(tg&&guid(m,'away'))return guid(m,'away')===tg;
  return norm(label(m,'away')).includes(norm(team.team_name));
}
async function matchCounts(ids){
  if(!ids.length)return new Map();
  const s=await client(),{data,error}=await s.rpc('get_match_presence_counts',{p_match_ids:ids.map(Number)});
  if(error)throw error;
  return new Map((data||[]).map(x=>[String(x.foy_match_id),{players:Number(x.player_count)||0,cars:Number(x.car_count)||0}]));
}
function injectCss(){
  if($('finalFixCssV4'))return;
  const s=document.createElement('style');s.id='finalFixCssV4';
  s.textContent=`
  ...
  .player-count-inline-final{display:inline!important;margin:0!important;font-size:inherit!important;line-height:inherit!important;font-weight:850!important;color:var(--navy)!important}
...
async function applyHeaderLogo(){
  ...
  headerBusy=true;
  try{
    let src='';const sel=selection();
    if(sel&&!String(sel.teamGuid||'').startsWith('all-'))src=sel.teamLogo||'';
    if(!src)src=sel?.clubLogo||'';
    try{
      const r=await fetch(`${API}/organisations/${ARGON}/teams`,{headers:{Accept:'application/json','X-FederationID':FED},cache:'no-store'});
      if(r.ok){
        const rows=(await r.json())||[],real=(Array.isArray(rows)?rows:[]).filter(x=>x?.guid&&!String(x.guid).startsWith('all-'));
        if(sel&&!String(sel.teamGuid||'').startsWith('all-'))src=real.find(x=>String(x.guid)===String(sel.teamGuid))?.logoUrl||src;
        else src=real.find(x=>x.logoUrl)?.logoUrl||src;
      }
    }catch{}
    if(src){
      const current=mark.querySelector('img')?.getAttribute('src')||'';
      if(current!==src){mark.classList.add('has-team-logo');mark.innerHTML=`<img src="${esc(src)}" alt="SV Argon">`}
    }else if(mark.classList.contains('has-team-logo')){
      mark.classList.remove('has-team-logo');mark.textContent='●';
    }
...
async function renderPresence(force=false){
  ...
  try{
    ...
    if(!c){target.innerHTML='';return}
    const byId=new Map(),today=new Date().toISOString().slice(0,10);
    const playerIds=new Set(c.playerTeams.map(x=>String(x.id)));
    const trainerIds=new Set(c.trainerTeams.map(x=>String(x.id)));
    const union=new Map();
    ...
    for(const tm of union.values()){
      for(const m of await teamMatches(tm.foy_team_guid)){
        ...
        const h=guid(m,'home'),a=guid(m,'away'),tg=String(tm.foy_team_guid||'');
        if(tg&&h!==tg&&a!==tg)continue;
        const key=String(m.id),prev=byId.get(key);
        const entry=prev||{m,team:tm,isPlayer:false,isTrainer:false};
        entry.isPlayer=entry.isPlayer||playerIds.has(String(tm.id));
        entry.isTrainer=entry.isTrainer||trainerIds.has(String(tm.id));
        if(!prev||entry.isPlayer)entry.team=tm;
        byId.set(key,entry);
      }
    }

    const rows=[...byId.values()].sort((a,b)=>`${d(a.m.date)}T${t(a.m.startTime)}`.localeCompare(`${d(b.m.date)}T${t(b.m.startTime)}`));
    const s=await client();
    const [{data:attendance,error:ae},counts]=await Promise.all([
      s.from('attendance').select('foy_match_id,attending,driving,team_id').eq('member_id',c.member.id),
      matchCounts(rows.map(x=>Number(x.m.id)))
    ]);
    if(ae)throw ae;
    const am=new Map((attendance||[]).map(x=>[String(x.foy_match_id),x]));

    target.innerHTML=`<div class="club-card-panel personal-presence-card">
      <h2>Aanwezigheid</h2>
      <p>Ja/Nee geldt voor spelers. Bij uitwedstrijden is Ik rijd dezelfde registratie als in de Agenda.</p>
      <div class="club-list">${rows.length?rows.map(({m,team,isPlayer,isTrainer})=>{
        ...
        return `<div class="club-row">
          <strong>${esc(label(m,'home'))} — ${esc(label(m,'away'))}</strong>
          <small>${esc(shortDate(m.date))} · ${esc(t(m.startTime))} · ${esc(m.accommodationName||'')}</small>
          <small class="presence-counts">Spelers: ${cnt.players} · Auto's: ${cnt.cars}</small>
          <div class="attendance-choice">${yesNo}${drive}</div>
        </div>`;
      }).join(''):'<div class="club-empty">Geen komende wedstrijden voor jouw speler- of trainerteam(s).</div>'}</div>
    </div>`;

    target.querySelectorAll('[data-presence-att]').forEach(b=>b.onclick=async()=>{
      const yes=b.dataset.presenceAtt==='yes',old=am.get(String(b.dataset.match));
      const {error}=await s.rpc('set_match_attendance',{
        p_match_id:Number(b.dataset.match),p_team_id:b.dataset.team,p_attending:yes,p_driving:yes&&old?.driving===true
      });
      if(error)return toast(error.message);
      toast(yes?'Aanwezig: Ja':'Aanwezig: Nee');
      await renderPresence(true);scheduleCounts();
      document.dispatchEvent(new CustomEvent('basketball-attendance-changed',{detail:{matchId:Number(b.dataset.match)}}));
    });
    target.querySelectorAll('[data-presence-drive]').forEach(b=>b.onclick=async()=>{
      ...
      const {error}=await s.rpc('set_match_attendance',{
        p_match_id:Number(b.dataset.match),p_team_id:b.dataset.team,p_attending:attending,p_driving:next
      });
      if(error)return toast(error.message);
      toast(next?'Ik rijd':'Rijden uitgeschakeld');
      await renderPresence(true);scheduleCounts();
      document.dispatchEvent(new CustomEvent('basketball-attendance-changed',{detail:{matchId:Number(b.dataset.match)}}));
    });
  }catch(e){
    console.warn('Aanwezigheid kon niet worden geladen',e);
    target.innerHTML='<div class="club-card-panel personal-presence-card"><h2>Aanwezigheid</h2><div class="club-empty">Aanwezigheid kon niet worden geladen.</div></div>';
  }finally{presenceBusy=false}
}
function schedulePresence(force=false){
  clearTimeout(presenceTimer);presenceTimer=setTimeout(()=>renderPresence(force),60);
}
async function applyCounts(){
  ...
  countsBusy=true;
  try{
    const counts=await matchCounts(ids);
    for(const card of cards){
      const meta=card.querySelector('.meta');if(!meta)continue;
      let pc=card.querySelector('.player-count');
...
function scheduleCounts(){
  clearTimeout(countsTimer);countsTimer=setTimeout(applyCounts,50);
}
function observeLists(){
  for(const id of ['agendaList','gamesList']){
    const root=$(id);if(!root||root.dataset.countObserver==='1')continue;
    root.dataset.countObserver='1';
    new MutationObserver(()=>scheduleCounts()).observe(root,{childList:true,subtree:true,characterData:true});
  }
}
function init(){
  injectCss();applyHeaderLogo();observeLists();scheduleCounts();schedulePresence(false);
  ...
  document.addEventListener('basketball-attendance-changed',()=>{schedulePresence(true);scheduleCounts()});
  document.addEventListener('click',e=>{
    if(e.target.closest?.('[data-clubview="presence"]'))schedulePresence(true);
    if(e.target.closest?.('#syncBtn'))setTimeout(()=>{scheduleCounts();schedulePresence(false)},200);
  },true);
  $('teamSelect')?.addEventListener('change',()=>setTimeout(()=>{applyHeaderLogo();scheduleCounts()},120));
}
